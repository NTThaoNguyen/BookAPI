...
import com.example.book.dto.OrderDetailDTO;
...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    public List<OrderDTO> getOrdersByUserId(String userId) { //??? Long ?
        return orderRepository.findByUser_Username(userId).stream().map(OrderDTO::new).collect(Collectors.toList());    }

    @Override
    public OrderDTO getOrderByUsernameAndOrderId(long orderId) {
        Order order = orderRepository.findById(orderId).orElseThrow(()-> new NotFoundException("Order not exists"));
        return new OrderDTO(order);
    }
